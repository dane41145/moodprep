...
function buildPresetPrompt(preset: GeminiPresetId, issues: IssueType[], backdrop: string | null) {
  ...
  if (preset === 'concentric') {
    return `This image is a printed roundel, label or coaster made of nested shapes — an outer disc or border, one or more printed rings or bands, and a central field — and the nesting is out of register: the inner assembly sits off-centre inside the outer one, so the band of the outer colour around it is visibly thicker on one side than on the opposite side. Your one job is to correct that register.

Do it in this order. First, find the outer silhouette of the whole piece and take its exact centre. Second, treat EVERYTHING inside the outermost band — every inner ring, every band of colour, every line of curved lettering that sits on those rings, the central panel, and every illustration, emblem and word printed on any of them — as one rigid group. Third, translate that whole group, unchanged and undistorted, until its centre lands exactly on the centre of the outer silhouette. The group must not be scaled, rotated or reshaped and nothing inside it may shift relative to anything else inside it: the ring text keeps its exact place on its ring and the emblem keeps its exact place on its panel. Only the group's position changes. After the move, every concentric element — the outer silhouette, each printed ring, rule, border and roundel, and the central field — shares a single common centre, and any curved lettering sits on an arc concentric with those rings, holding a constant distance from the edge all the way around.

The test is measurable and you must apply it before you answer. Take the band of the outer colour that lies between the outer silhouette and the first inner ring and measure its width at the top, at the bottom, at the left, at the right, and on both diagonals: all six measurements must come out equal, and the band must be exactly the same width the whole way round. Do the same for every band between neighbouring rings further in. If any band is still thicker on one side than the other, the piece is still out of register and the result is wrong — move the group again and measure again. Returning the image with the same offset it arrived with is the specific failure this instruction exists to prevent. If the whole piece sits within the canvas, its margin to the canvas edge must be equal on all four sides as well, so the piece sits at the exact centre of the frame.

This is a deliberate regularisation, not a restoration. These pieces were printed on fast presses and the physical object really is out of register, so the reference genuinely does show the inner assembly nudged to one side; reproducing that faithfully is exactly what is not wanted here. Return the design as the press intended it, not as it actually came off the press. Where the reference and true concentricity disagree, concentricity wins, without exception.

Move, and do nothing else. Do not redraw, restyle, resize, rescale, straighten, rotate, recolour, clean, sharpen, denoise or re-letter anything. Keep every colour, shape, letter, line, thickness, proportion, texture and edge exactly as it is, including any grain, wear, damage or printing artefacts — they are not your concern in this task. Add nothing and remove nothing. The move uncovers a sliver of the outer band on one side and covers a sliver on the other: fill the uncovered sliver with the outer band's own flat colour so the band is continuous, leaving no seam, halo, outline, shadow or ghost where the group used to sit. Return only the corrected image at the same aspect ratio.`
  }
  if (preset === 'outpaint') {
...
Concentricity and ring placement (the most common failure by far, and it is not caused by the camera): these pieces were printed on fast presses and the physical object is very often genuinely out of register, so on the real coaster the inner disc sits slightly off-centre inside the outer ring and the band of colour between them is visibly thicker on one side than the other. Reproducing that faithfully is wrong here. Correct it explicitly: take the centre of the outer silhouette, treat everything inside the outermost band — every inner ring and band, the central disc or panel, and every word, emblem and illustration printed on them — as one rigid group, and move that whole group, unchanged, until its centre coincides with the outer centre; nothing inside the group shifts relative to anything else inside it. Every concentric element — the outer silhouette, each printed ring, rule, border and roundel, and the central field — must share one single common centre. The band between any two neighbouring rings must therefore be exactly the same width the whole way round: measure it at the top, at the bottom, at the left, at the right, and on both diagonals, and all six measurements must come out equal. A ring that is thicker at the bottom than the top, an inner circle nudged towards one side, a border that crowds one edge of the piece while leaving a wide gap at the other, or a design whose centre does not coincide with the silhouette's centre is wrong however plainly the reference shows it. Curved lettering must sit on an arc concentric with those same rings, holding a constant distance from the edge all the way around. Treat this as a deliberate regularisation rather than a restoration: return the design as the press intended it, not as it actually came off the press. Returning the piece with the same offset the photograph shows is a failure of this task, so measure those bands before you answer and move the group again if they are unequal. This requirement overrides fidelity to the reference. Fidelity governs the wording, the illustration, the typography and the colours; it does not govern where a ring sits.
